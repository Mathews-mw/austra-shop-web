...
export function StarsRatingView({ totalStars = 5, score, showAverage, iconsSize = 20 }: IStarsRatingView) {
	return (
		<div className="text-primary relative inline-flex gap-1 text-left">
			{[...new Array(totalStars)].map((_, index) => {
				...
				return (
					<div key={index} title={showAverage ? `Média: ${score}` : undefined} className="relative">
						...
						<div className={twMerge(`${showEmptyIcon ? 'text-primary' : 'text-primary'}`)}>
							{showEmptyIcon ? <Star size={iconsSize} /> : <Star size={iconsSize} className="fill-primary" />}
						</div>
					</div>
				);
			})}
